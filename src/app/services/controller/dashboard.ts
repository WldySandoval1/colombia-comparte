  ...
  async getMetrics(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (rol === 'superadmin') {
        ...
        const [solicitudesPendientes, testimoniosPublicados, noticiasActivas] = await Promise.all([
          // Agrupa solicitudes pendientes por país
          SolicitudModel.aggregate([
            { $match: { estado: 'pendiente' } },
            { $group: { _id: '$pais', total: { $sum: 1 } } },
            { $lookup: { from: 'pais', localField: '_id', foreignField: '_id', as: 'pais_info' } },
        ...
        if (!pais) {
          return res.status(400).json({ ok: false, error_message: 'El usuario no tiene país asignado' });
        }
      ...
      return res.status(500).json({ ok: false, error_message: 'Error interno del servidor' });
    }
  }
}